...
export function CardOferta({ oferta }: Props) {
  const descontoLabel = oferta.desconto ? `-${oferta.desconto}%` : undefined
  return (
    <div className="group relative rounded-xl border border-slate-200 bg-white overflow-hidden shadow-sm transition-all hover:shadow-xl hover:border-indigo-200">
      ...
      <div className="p-4">
        <Link href={`/oferta/${oferta.id}`} className="mb-2 block">
          <h3 className="line-clamp-2 text-sm font-semibold text-slate-900 transition-colors group-hover:text-indigo-600">
            {oferta.titulo}
          </h3>
        </Link>
        ...
        <div className="mb-3 flex items-baseline gap-2">
          ...
          {oferta.precoAnterior && <span className="text-sm text-slate-400 line-through">{oferta.precoAnterior}</span>}
        </div>
        ...
        <div className="flex items-center justify-between border-t border-slate-100 pt-3">
          ...
          <a
            ...
            className="flex items-center gap-1 rounded-md bg-indigo-50 px-3 py-1.5 text-xs font-semibold text-indigo-600 hover:bg-indigo-100 transition-colors"
          >
            Ver oferta
